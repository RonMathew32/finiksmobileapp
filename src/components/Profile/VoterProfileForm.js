...
const VoterProfileForm = React.memo(({ onSaveData, canvass = false, userData }) => {
  ...
  const initialData = useMemo(() => ({
    ...
    email: userData?.EMAIL ?? userData?.EMAIL2 ?? userData?.EMAIL3 ?? userData?.email ?? '',
    preferredName: '',
  ...
  const onPressClearInput = useCallback((key) => {
    if (canvass) {
